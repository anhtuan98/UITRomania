...
public class Copy1OfRecognition {
    ...
    Copy1OfRecognition(String filename){
        nnetwork = new CvANN_MLP();
        nnetwork.load(filename);
    }
    public void scaleDownImage(Mat originalImg,Mat output )
    {
        int i=0;
        for(int x=0;x<32;x++)
        {
            for(int y=0;y<24 ;y++)
            {
                int yd =(int) Math.round((float)(y*originalImg.cols()/24));
                int xd =(int) Math.round((float)(x*originalImg.rows()/32));
    ...
    public void scaleDownImage( )
    {
        ...
        for(int x=0;x<32;x++)
        {
            for(int y=0;y<24 ;y++)
            {
                int yd =(int) Math.round((float)(y*datasubmat.cols()/24));
                int xd =(int) Math.round((float)(x*datasubmat.rows()/32));
                float [] v= new float[1];
    ...
    public String reg(Mat img){
        ...
        try{
            ...
            Imgproc.threshold(data, data, 100,255,0);
            ...
            maxIndex = 36;
            //Log.e("Recognition Error"+ex.toString());
        }finally{
            if(maxIndex!=36)
